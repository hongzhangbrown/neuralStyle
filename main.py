...
def content_loss(img,content_img):
	img_content_layer,_ = conv_layers(img)
...
def gram_one_layer(one_layer):
	one_layer_dim = np.array(img_layer_dim.get_shape().as_list())
	num_filter = img_layer_dim[-1]
...
def style_loss(img,style_img):
	_,img_style_layers = conv_layers(img)
...
def run(sess,content_path = 'laska.png',style_path = 'tofu.jpg'):
	content_img = read_images(content_path)
	style_img = read_images(style_path)
	img = tf.Variable(tf.truncated_normal(content_img.shape, stddev=20), trainable=True, name='gen_image')
	num_iter = 100
